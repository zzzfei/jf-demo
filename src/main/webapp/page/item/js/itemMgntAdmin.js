...
function initGrid(){
    ...
    store = Ext.create('Ext.data.Store', {
        ...
        proxy: {
            type: 'ajax',
            url: rootPath + '/icItemController/getIcItemForList?itemApprState=2',
            data: data,
            reader: {
                type: 'json',
                // 指定根节点使用的数据
    ...
    queryGrid = Ext.create('Ext.grid.Panel', {
        ...
        viewConfig: {
            ...
            forceFit : true,
            emptyText : "<font class='emptyText'><h1>暂时没有待审批的商品</h1></font>",
            autoScroll:true,
            scrollOffset:-10
        }
    });
...
function buttonRender(value, meta, record, rowIndex, colIndex, store) {
    ...
    var detail = '<button onclick="detail('+itemId+')">查看详情</button><br><br>';
    ...
    var apprStop = '<button onclick="apprStop('+itemId+')">审批不通过</button><br><br>';
    ...
    if(itemApprState == '2'){
    	// 待审判状态可以进行审批（通过和不通过）
    	// 通过后为下架状态
    	returnValue += detail + apprPass + apprStop;
    }else if(itemApprState == '4'){
    	// 审判不通过商品可以进行修改，修改完之后变为下架状态
    	returnValue += detail + modify;
    }
